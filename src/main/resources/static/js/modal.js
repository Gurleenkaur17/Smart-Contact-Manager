var viewContactModal = document.getElementById('view_contact_modal');

var options = {
    placement: 'bottom-right',
    backdrop: 'dynamic',
    backdropClasses: 'bg-gray-900/50 dark:bg-gray-900/80 fixed inset-0 z-40',
    closable: true,
    onHide: () => {
        console.log('modal is hidden');
    },
    onShow: () => {
        console.log('modal is shown');
    },
    onToggle: () => {
        console.log('modal has been toggled');
    },
};

var instanceOptions = {
    id: 'modalEl',
    override: true
  };

var contactModal = new Modal(viewContactModal, options, instanceOptions);

function openContactModal() {
    contactModal.show();
}

function hideContactModal() {
    contactModal.hide();
}
const baseUrl = "http://localhost:8080";
async function loadContactData(id){
    console.log(id);

    try{
        const data =  await (
            await fetch(`${baseUrl}/api/view/${id}`)
            ).json();
            document.getElementById('contact_name').innerHTML = data.name;
            document.getElementById('contact_email').innerHTML = data.email;
            document.getElementById('contact_number').innerHTML = data.phoneNumber
            document.getElementById('contact_address').innerHTML = data.address;
            
            document.getElementById('contact_description').innerHTML = data.description;
            var favouriteContainer = document.getElementById('contact_favourite');
            if(data.websiteLink != null && data.websiteLink != '' && data.websiteLink != "na"){
                var aTag = document.createElement('a');
                aTag.href = data.websiteLink;
                aTag.textContent = 'Visit Website'; // Or use the URL itself if desired
                aTag.target = '_blank'; // Open link in a new tab
                aTag.className = 'text-blue-500 hover:underline';
                document.getElementById('contact_website').appendChild(aTag);
            }
            if(data.linkedInLink != null && data.linkedInLink != '' && data.linkedInLink != "na"){
                var aTag = document.createElement('a');
                aTag.href = data.linkedInLink;
                aTag.textContent = 'Visit LinkedIn'; // Or use the URL itself if desired
                aTag.target = '_blank'; // Open link in a new tab
                aTag.className = 'text-blue-500 hover:underline';
                document.getElementById('contact_linkedIn').appendChild(aTag);
            }
            if(data.favourite == true) {
                for (var i = 0; i < 5; i++) {
                    var starIcon = document.createElement('i');
                    starIcon.className = 'fa-solid fa-star';
                    favouriteContainer.appendChild(starIcon);
                }
            }

            if(data.picture){
                document.getElementById('contact_image').src = data.picture;
            }
            else{
                document.getElementById('contact_image').src = "data:image/png;base64,";
            }
            openContactModal();

    }
    catch(error){
        console.log(error);
        
    }

}

async function deleteContact(id) {
    Swal.fire({
        title: "Are you sure?",
        text: "You won't be able to revert this!",
        icon: "warning",
        showCancelButton: true,
        confirmButtonColor: "#3085d6",
        cancelButtonColor: "#d33",
        confirmButtonText: "Yes, delete it!"
      }).then((result) => {
        if (result.isConfirmed) {
            const url = `${baseUrl}/user/contacts/delete/${id}`
            window.location.replace(url);
          Swal.fire({
            title: "Deleted!",
            text: "Your file has been deleted.",
            icon: "success"
          });
        }
      });
}
